//定义一个区域图类：
function GooFlow(bgDiv,property,ajaxHtml){
...
GooFlow.prototype={
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			This.$workArea.parent().one("mousedown",function(e){
				if(e.button==2)return false;
				This.setName(This.$textArea.data("id"),This.$textArea.children("select").find("option:selected").text(),This.$textArea.children("select").find("option:selected").val(),"node");
				console.log(This.$textArea.children("select").find("option:selected").val())
				This.$textArea.val("").removeData("id").hide();
			});
		});
		//width:$(this).width()+26,height:$(this).height()+6,
...
//          }
	...
	//设置结点/连线/分组区域的文字信息
	setName:function(id,name,SceneID,type){
		var oldName;
		if(type=="node"){//如果是结点
			if(!this.$nodeData[id])	return;
			...
			this.$nodeData[id]['SceneID'] = SceneID
...
jQuery.extend({
	createGooFlow:function(bgDiv,property,ajaxHtml){
		return new GooFlow(bgDiv,property,ajaxHtml);
	}
}); 